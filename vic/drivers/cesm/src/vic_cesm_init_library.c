...
initialize_l2x_data(void)
{
    ...
    // NOTE: l2x values that need to be populated are initialized with fill 
    // values, and all other l2x fields are populated with zeros (to be 
    // consistent with VIC4 coupling)
...
vic_initialize_temperature(void)
{
    ...
    for (i = 0; i < local_domain.ncells_active; i++) {
	l2x_vic[i].l2x_Sl_t = soil_con[i].avg_temp;
    }
}	
